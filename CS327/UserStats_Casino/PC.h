...
public:
    ...
    void listInventory() const {
        for (int i = 0 ; i < 10 ; i++)
        {
            if (inventory[i])
            {
                mvprintw(i + 3, 0, "%d: %s", i, inventory[i]->name.c_str());
            }
        }
    }
    ...
    int getModifiedSpeed() const {
        int baseSpeed = speed ;
        int totalWeight = getModifiedWeight();

        for (int i = 0 ; i < 12 ; i++)
        {
            if (equipment[i])
            {
                baseSpeed += equipment[i]->speed;
            }
        }

        double weightEffect = 5.0 - (totalWeight / 20.0);
        
        baseSpeed += static_cast<int>(weightEffect);
    ...
    int getModifiedHit() const {
        int total = this->hit;
        for (int i = 0; i < 12; i++) {
            if (equipment[i]) {
                total += equipment[i]->hit;
            }
        }
        return total;
    }

    int getModifiedDodge() const {
        int dodge = this->dodge;
        int totalWeight = getModifiedWeight();
    
        for (int i = 0; i < 12; i++) {
            if (equipment[i]) {
                dodge += equipment[i]->dodge;
            }
        }
    
        double weightEffect = 3.0 - (totalWeight / 30.0);  
    
        dodge += static_cast<int>(weightEffect);
    ...
    int getModifiedWeight() const {
        int total = 0;
        for (int i = 0; i < 12; i++) {
            if (equipment[i]) {
                total += equipment[i]->weight;
            }
        }
        for (int i = 0; i < 10; i++) {
            if (inventory[i]) {
