...
# Try 50 models with randomized params
for trial in range(50):
    print(f"\n=== Trial {trial+1} ===")

    model = xgb.XGBClassifier(
        n_estimators=np.random.randint(150, 350),
        max_depth=np.random.randint(4, 12),
        learning_rate=np.random.uniform(0.01, 0.15),
...
# Final result
print(f"\n\n🏆 Best XGBoost Model = Trial {best_trial} | Avg Score = {best_score:.2f}")
joblib.dump(best_model, "best_xgbracket_model.pkl")
print("✅ Model saved to best_xgbracket_model.pkl")
