...
class Edge<V, C extends Comparable<? super C> > implements Comparable<Edge<V, C>>
   {
     ...
     public int compareTo( Edge<V, C> other )
     {
       ...
       // Negative if other's cost is more than current edge, vice versa for positive.
     }
...
public static <V> HashMap<V, Integer> Dijkstra(DiGraph<V> G, V source, V dst) {
  ...
  // (key, value) pairs where key = vertex, value = distance of shortest edges to reach it.
  LinkedStack<V> path = new LinkedStack<V>();

  // Initialize all distnces to infinity and all predecessors to null.
  // This is first step for dijikstras. 
  for (V v : G.vertices()) {
    dist.put(v, Integer.MAX_VALUE);
    pred.put(v, null);
  }
  // Set the initial source vertex distance to itself to ZERO.
  dist.put(source, 0);

  // A min heap of Edge objects. 
  // Add the source vertex with a distance of 0 first.
  ...
  /*
   * This while loop is responsible for the core of the Dijikstra's algorithm.
   * The loop updates the shortest path as vertices are found from the source vertex onwards
   * based on the differently weighted distances between certain vertices.
   * As these shortest paths are built, the loop makes sure to explore every vertice and 
   * edge to ensure that it is the shortest possible path and distance between every pair of vertices.
   * The loop uses a priority quere to decide on which vertec should be focused on next based on shortest distance
   * and the timing on whether the vertice had been explored.  
   * 
   */
  while (priq.isEmpty() == false) {
    // Remove edge with lowest cost from priority queue, and extract vertex from the edge and set curVertex to it.
    Edge<V, Integer> curEdge = priq.removeMin();
    V curVertex = curEdge.getVertex();
    int currentDist ;

    if (vset.contains(curVertex) == false) {

      // Ensures that vertex is only computed once regarding djikstras in that it only gets
      // added to the hash set (vset) once. Sets current distance to the current vertex's distance.
      vset.add(curVertex);
      currentDist = dist.get(curVertex);

      // Iterates through every edge that is adjacent to the current vertice,
      // extracting the cost/distance to reach the neighbor and adding it to newDist.
      // If the newDist value is lower than the already set distance value to get to the neighbor,
      // then the dist hashMap value for the vertice is updated, the predecessor of the neighbor
      // is set to the curVertex, and adds the edge with the new cost to the priority queue.
      for (Edge<V, Integer> e : G.adjacentTo(curVertex)) {
        int newDist ;
        int cost ;
        V neighbor ;
        cost = e.getCost();
        newDist = currentDist + cost;
        neighbor = e.getVertex();
        if (newDist < dist.get(neighbor)) {
          pred.put(neighbor, curVertex);
          dist.put(neighbor, newDist);
          priq.add(new Edge<V, Integer>(neighbor, newDist));
        }
      }
    }
  }
  ...
  // Reconstructs the shortest pssible path from dst (the destination vertex) to the source vertex.
  // Does this by tracing through predecessors that were established during the while loop, which were
  // all guranteed to be the shortest possible distance.
  // LinkedStack path stores the stack of vertices.
  for (V vert = dst; vert != null; vert = pred.get(vert)) {
    path.push(vert);
  }


  // Assigns the graph's predecessor hashMap to the temporary predecessors that has been maintained.
  // Also assigns the graph's path hashMap to the temporary path hashmap, and returns the total distance
  G.vertexPred = pred;
  G.path = path;
  return dist;
}
