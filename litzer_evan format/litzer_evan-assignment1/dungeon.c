...
int createRooms(char dungeonMap[21][80])
{
    int attempts = 1 ;
...
int createUpStaircases(char dungeonMap[21][80], int totalRooms)
{
...
int createDownStaircases(char dungeonMap[21][80], int totalRooms)
{
...
void generatePath(char dungeonMap[21][80], int room1X, int room1Y, int room3X, int room3Y) {
    int x = room1X;
    int y = room1Y;
...
void calculatePath(char dungeonMap[21][80], int totalRooms)
{
    ...
    for(int z = 1 ; z < totalRooms ; z++)
    {
        ...
        for(int a = 0 ; a < z ; a++)
        {
            ...
            if(nearestRoom != -10)
            {
                ...
                generatePath(dungeonMap, room1X, room1Y, room3X, room3Y);
...
int main() {
    ...
    int totalRooms = createRooms(dungeonMap);

    int totalUpStaircases = createUpStaircases(dungeonMap, totalRooms) ;

    int totalDownStaircases = createDownStaircases(dungeonMap, totalRooms) ;

    (void)totalUpStaircases;
    (void)totalDownStaircases;

    calculatePath(dungeonMap, totalRooms) ;
