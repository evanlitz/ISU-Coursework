...
void generatePlayerPosition(char dungeonMap[21][80], int totalRooms, int *playerX, int *playerY)
{
    int randomRoom = rand() % totalRooms ;
...
int main(int argc, char *argv[]) {
    ...
    else 
    {
        ...
        generatePlayerPosition(dungeonMap, totalRooms, &playerX, &playerY) ;
        calculateDistanceMap(dungeonMap, hardness, playerX, playerY, distanceMapNonTunnel, false);
        calculateDistanceMap(dungeonMap, hardness, playerX, playerY, distanceMapTunnel, true);
    }

    printDungeonMap(dungeonMap); 
    printDistanceMap(distanceMapNonTunnel, dungeonMap, playerX, playerY, false) ;
    printDistanceMap(distanceMapTunnel, dungeonMap, playerX, playerY, true) ;
